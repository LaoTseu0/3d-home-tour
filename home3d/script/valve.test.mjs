...
import {
  ...
  isValvablePipe,
} from '../src/lib/valve.js'
import { PIPE_KIND, PIPE_SECTIONS, slopedPoints } from '../src/lib/plumbing.js'
import { closestOnPath } from '../src/lib/routing.js'
...
const pipe = (points, sectionKey = 'cuivre16', extra = {}) => {
  ...
  return {
    ...
    plane: { type: 'run', origin: points[0], u: [1, 0, 0], v: [0, 1, 0], normal: [0, 0, 1] },
  }
}
...
describe('splitPipeAt — garde-fous', () => {
  ...
  it('chemin dégénéré ou objet absent → null', () => {
    assert.equal(splitPipeAt({ params: { points: [[0, 0, 0]] } }, [0, 0, 0]), null)
...
describe('valveBodyLength / valveMesh', () => {
  it('corps = facteur × plus grand côté, plancher pour le petit cuivre', () => {
    assert.ok(close(valveBodyLength({ largeur_m: 0.1, hauteur_m: 0.1 }), VALVE_BODY_FACTOR * 0.1))
    assert.ok(close(valveBodyLength({ largeur_m: 0.012, hauteur_m: 0.012 }), VALVE_BODY_MIN))
  })
  ...
  it('corps centré sur la coupe, section sur-dimensionnée (collier)', () => {
    const params = { centre: [1, 0.3, 0], dir: [1, 0, 0], ...PIPE_SECTIONS.evac100.dims }
...
describe('dropFittingsAtValves — pas de manchon sous la vanne', () => {
  const valve = {
    params: { centre: [2, 0.3, 0], dir: [1, 0, 0], ...PIPE_SECTIONS.cuivre16.dims },
  }

  it('un raccord au point de coupe est supprimé, les autres restent', () => {
    const fittings = [
      { type: 'manchon', point: [2, 0.3, 0] }, // jonction des deux tronçons
      { type: 'te', point: [5, 0.3, 0] }, // vraie jonction ailleurs
    ]
  ...
  it('sans vanne (ou liste vide) → raccords inchangés', () => {
    const fittings = [{ type: 'manchon', point: [2, 0.3, 0] }]
...
describe('isValvablePipe', () => {
  it('tuyau routé valide → oui ; câble, tuyau dégénéré, absent → non', () => {
    assert.ok(isValvablePipe(pipe([[0, 0, 0], [1, 0, 0]])))
    assert.ok(!isValvablePipe({ kind: 'elec.cable', params: { points: [[0, 0, 0], [1, 0, 0]] } }))
    assert.ok(!isValvablePipe({ kind: PIPE_KIND, params: { points: [[0, 0, 0]] } }))
