...
import { worldToScreen, meshRefsNear } from './snapRefs.js'
import { referencePoints } from './editRegistry.js'
import { parseVcbLength } from './vcb.js'
import { isChainVisible } from './appearance.js'
 ...
 *     refPoint,  // point MONDE solidaire de la face tirée (poignée ou point
 *                //   cliqué) — ancre de la mesure et du snapping
 *   }
    ...
    (p, cursor, rect) => {
      ...
      if (glb?.scene) {
        ...
        if (hit) {
          ...
          const refs = meshRefsNear(hit, hp, camera, rect)
      ...
      const valid = candidates.filter((c) => c.value >= MIN_VALUE)
      return pickBestSnap(valid, cursor, SNAP_THRESHOLD_PX)
    },
    [raycaster, camera]
  )
