...
// de three, pas de react) → testable seul (script/valve.test.mjs).
//
...
import { closestOnPath, dedupePath, dist, runMesh } from './routing.js'
import { PIPE_KIND, slopedPoints } from './plumbing.js'

export const VALVE_KIND = 'plomberie.valve'
...
const roundPt = (p) => [round(p[0]), round(p[1]), round(p[2])]
...
export function valveBodyLength(params) {
 ...
 * @param {{kind?:string, params:object}} pipe objet app tuyau
 * @param {number[]} worldPoint point cliqué (monde, sur la surface du tuyau)
 * @returns {{valve:{kind, params, plane}, runs:object[]}|null}
 */
export function splitPipeAt(pipe, worldPoint) {
  ...
  const lower = ([x, y, z]) => [x, round(y - drop), z]
  ...
  return {
    valve: {
      ...
      plane: { type: 'run', origin: centre, u: [1, 0, 0], v: [0, 1, 0], normal: [0, 0, 1] },
    },
    runs: [
...
// lib/routing) → la tige de manœuvre monte au-dessus du tuyau ; repli sur X
// quand l'axe est ~vertical.
function valveBasis(dir) {
  let t = norm(dir)
 ...
 * pièce est un mini-run de 2 points (runMesh, lib/routing, même esthétique
 * low-poly que runs et raccords). Tableaux plats prêts pour un BufferGeometry.
 * @param {{centre:number[], dir:number[], largeur_m:number, hauteur_m:number}} params
 * @returns {{position:number[], index:number[]}}
 */
export function valveMesh(params) {
  ...
  const pieces = [
    ...
    { a: sub(top, half(axis, handleLen)), b: add(top, half(axis, handleLen)), side: stemSide },
  ]
 ...
 * @param {{point:number[]}[]} fittings sortie de detectFittings
 * @param {{params:{centre:number[]}}[]} valves objets vanne
 * @returns {{point:number[]}[]} raccords conservés
 */
export function dropFittingsAtValves(fittings, valves) {
  const list = valves ?? []
  ...
  return (fittings ?? []).filter(
    (f) =>
      !list.some(
        (v) =>
          v.params?.centre && dist(f.point, v.params.centre) <= valveBodyLength(v.params) / 2
      )
  )
}

/** Un objet app est-il coupable par l'outil Vanne ? (tuyau routé valide) */
export function isValvablePipe(obj) {
